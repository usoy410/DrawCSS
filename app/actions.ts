...
export async function processImageToCode(base64Image: string, framework: string = "vanilla", styling: string = "tailwind") {
    if (!process.env.GEMINI_API_KEY) {
        throw new Error("GEMINI_API_KEY is not set");
    }

    const model = getGeminiModel(framework, styling);
